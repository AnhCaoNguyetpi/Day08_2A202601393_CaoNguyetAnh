...
Nhóm gồm 3 thành viên nên công việc được chia theo ba khối độc lập nhưng có điểm kiểm tra chéo. Mỗi thành viên chịu trách nhiệm chính cho một khối và cùng review phần tích hợp cuối.

| Thành viên                  | MSSV        | Vai trò                                             | Nhiệm vụ chính                                                                                                                                                               | Đầu ra phụ trách                                                                                                                  | Trạng thái |
| ----------------------------- | ----------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| **Cao Nguyệt Ánh**    | 2A202601393 | **Team Leader, AI & RAG Integration Engineer** | Điều phối tiến độ; Task 7–10 (reranking, fallback, retrieval pipeline, generation); intent routing, guardrail, citation và tích hợp toàn hệ thống                  | `task7_reranking.py` → `task10_generation.py`, `intent_router.py`, `policy_case_reasoning.py`, kiến trúc và demo pipeline | Hoàn thành |
| **Bùi Trung Hiếu**    | 2A202601281 | **Data & Retrieval Engineer**                  | Task 1–6: kiểm tra 4 PDF, chuẩn hóa Markdown, chunking/indexing, semantic search và lexical search; kiểm tra metadata và chất lượng dữ liệu                         | `data/`, `task1_collect_legal_docs.py` → `task6_lexical_search.py`, chỉ mục 218 đoạn dữ liệu                             | Hoàn thành |
| **Trần Thị Vân Anh** | 2A202601411 | **Frontend, Evaluation & QA Engineer**         | Hoàn thiện Streamlit UX; hiển thị nguồn/confidence; xây golden dataset, kiểm thử 50 câu, guardrail/adversarial tests, báo cáo evaluation và tài liệu hướng dẫn | `app.py`, `group_project/evaluation/`, `tests/`, `PRODUCTION_RAG_DESIGN.md`, báo cáo và demo UI                            | Hoàn thành |

#### Quy tắc phối hợp và kiểm tra chéo

- Cao Nguyệt Ánh review kết nối end-to-end và bảo đảm các module tuân thủ cùng response schema.
- Bùi Trung Hiếu kiểm tra nguồn trích dẫn, metadata, section và chất lượng các đoạn được truy hồi.
- Trần Thị Vân Anh chạy toàn bộ regression test, kiểm tra UX và ghi nhận kết quả trước khi demo.
- Cả ba cùng chạy `pytest tests -q`, thử ứng dụng trên `localhost:8501` và duyệt README trước khi nộp.
